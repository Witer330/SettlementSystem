...
const calculateSpecPrice = async (specId: number): Promise<number | null> => {
  ...
  // 解析规格参数
  const dimensions = typeof spec.dimensions === 'string' ? JSON.parse(spec.dimensions) : spec.dimensions;
  ...
  for (const coeff of coefficients) {
    const matched = matchCoefficient(coeff, { dimensions, material, craft, difficulty: spec.difficulty });
    if (matched) {
