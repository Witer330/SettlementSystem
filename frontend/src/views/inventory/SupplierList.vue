<template>
  <div class="page-container">
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑供应商' : '新增供应商'" width="520px">
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await supplierApi.getList(queryParams)
...
const handleDelete = async (row: Supplier) => {
  await ElMessageBox.confirm(`确定要删除供应商"${row.name}"吗？`, '确认删除', { type: 'warning' })
